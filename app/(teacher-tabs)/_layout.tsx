...
// member or a Site Administrator is using this view temporarily instead of
// a real teacher.
...
export default function TeacherTabLayout() {
    ...
    // OKAGE staff or a Site Administrator can preview the teacher view to
    // check that their content edits/school data show up correctly. Real
    // teachers never see this banner. True only when the currently
    // logged-in user is an 'okage' or 'site_admin' role AND has explicitly
    // switched their active_view to 'teacher' (see the (okage-tabs) and
    // (site-admin-tabs) account screens for where that toggle lives).
    const [isPreviewing, setIsPreviewing] = useState(false);
    // Remembers which "real" role is doing the previewing, so the banner
    // text and the "return to ___" action send them back to the right
    // place. Defaults to 'okage'; corrected to 'site_admin' once the
    // profile loads if that's what it turns out to be.
    const [previewingRole, setPreviewingRole] = useState<'okage' | 'site_admin'>('okage');
    // Tracks whether the "return to ___ view" action is currently in
    ...
    useEffect(() => {
        ...
        async function checkViewMode() {
            try {
                ...
                // true: this account's real role is 'okage' or 'site_admin'
                // (not an actual teacher), AND they've chosen to preview
                // the teacher view specifically.
                const isOkage = profile.role === 'okage';
                const isSiteAdmin = profile.role === 'site_admin';
                setPreviewingRole(isSiteAdmin ? 'site_admin' : 'okage');
                setIsPreviewing((isOkage || isSiteAdmin) && profile.active_view === 'teacher');
                setChecked(true);
            } catch (error) {
    ...
    // Handles the banner's tap action: switch the previewing user's
    // active_view back to their own real view ('okage' or 'site_admin') in
    // the database, then navigate them back to their real tabs.
    const handleReturnToRealView = async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('profiles')
                .update({ active_view: previewingRole })
                .eq('id', user.id);

            if (error) throw error;

            setIsPreviewing(false);
            router.replace((previewingRole === 'site_admin' ? '/(site-admin-tabs)/' : '/(okage-tabs)/') as any);
        } catch (error) {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            {/* Never shown to an okage/site_admin user previewing this
                view -- only to a genuine teacher. `checked` (the access
                guard above) has already resolved by the time this renders,
                since it gates the early spinner return. */}
            <OnboardingTour tourId="teacher" active={!isPreviewing} ready={checked} />

            {/* The orange preview banner only renders at all when
                isPreviewing is true — otherwise this entire block is
                skipped. */}
            {isPreviewing && (
                ...
                <View style={{ backgroundColor: theme.accent, paddingTop: insets.top }}>
                    <TouchableOpacity
                        style={styles.banner}
                        onPress={handleReturnToRealView}
                    ...
                    >
                        {switchingBack ? (
                            ...
                            <Text style={styles.bannerText}>
                                Previewing as Teacher — Tap to Return to {previewingRole === 'site_admin' ? 'Site Admin' : 'OKAGE'} View
                            </Text>
                        )}
                    </TouchableOpacity>
                </View>
                ...
                >
                    ...
                    <Tabs.Screen
                        ...
                        options={{
                            ...
                            tabBarIcon: ({ color, focused }) => (
                                <TourTarget id="teacher.classesTab">
                    ...
                    <Tabs.Screen
                        ...
                        options={{
                            title: 'Curriculum',
                            tabBarIcon: ({ color, focused }) => (
                                <TourTarget id="teacher.curriculumTab">
                    ...
                    <Tabs.Screen
                        ...
                        options={{
                            title: 'Reports',
                            tabBarIcon: ({ color, focused }) => (
                                <TourTarget id="teacher.reportsTab">
